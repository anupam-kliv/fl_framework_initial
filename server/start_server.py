...
parser.add_argument("--rounds", type=int, default = 1,
                     help = "Total number of communication rounds to perform")
parser.add_argument("--model_path",  default = "initial_model.pt",
                     help = "The path of the initial server model's state dict")
parser.add_argument("--epochs", type = int, default = 1,
                     help="Number of epochs each client should perform in each round")
parser.add_argument("--accept_conn",type = int,default = 1,
                    help = """If set to 1, connections will be accpeted even after FL has begun,
                     else if set to 0, they will be rejected.""")
parser.add_argument("--verify", type = int, default = 0,
                     help="""If the verification module should be run before each round.
                     Specify 1 for True or 0 for False""")
parser.add_argument("--threshold",type = float,default = 0,
                     help = """Minimum score clients must have in a verification round,
                     if verification is enabled. Between 0 and 1 inclusive""")
...
parser.add_argument("--dataset", type = str, default= "FashionMNIST",
                     help="datsset name. Use CUSTOME if you want to use local dataset ")
parser.add_argument("--niid", type = int,default=1, help="value should be [1, 5]")
parser.add_argument("--carbon", type = int,default=0,
                     help="set to 1 if carbon emission need to be track at client side")
